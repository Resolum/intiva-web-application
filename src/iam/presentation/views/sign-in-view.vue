...
import BaseInput from '@/shared/presentation/components/BaseInput.vue'
import BaseButton from '@/shared/presentation/components/BaseButton.vue'
import { useAuthStore } from '@/iam/application/auth.store.js'

const { t } = useI18n()
const router = useRouter()
const authStore = useAuthStore()
const { signIn, errors } = authStore
...
watch(errors, (newErrors) => {
  if (newErrors.length > 0) {
    const last = newErrors[newErrors.length - 1]
    globalError.value = last.message.startsWith('login.errors.')
      ? t(last.message)
      : last.message
  }
}, { deep: true })
 ...
 * Delegates to the auth store which handles validation, API call, and navigation.
 */
const handleLogin = async () => {
  isLoading.value = true
  globalError.value = ''
  await signIn(email.value, password.value, router)
